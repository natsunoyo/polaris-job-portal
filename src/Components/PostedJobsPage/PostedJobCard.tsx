...
const PostedJobCard = (props: any) => {
    ...
    return <Link to={`/posted-jobs/${props.id}`}>
        <div className={` rounded-xl p-2 border-l-8 border-l-purple-heart-400 ${props.id == id ? "bg-purple-heart-400 text-woodsmoke-950" : "bg-woodsmoke-900"}`}>
            ...
            <div className={`text-md ${props.id == id ? 'text-woodsmoke-900' : 'text-silver-sand-400'}  `}>{props.jobStatus == 'DRAFT' ? "В чернетках" : props.jobStatus == "CLOSED" ? "Неактивна" : "Опублікована"} {timeAgo(props.postTime)}</div>
        </div>
    </Link >
}
